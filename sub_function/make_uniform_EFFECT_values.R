make_uniform_EFFECT_values <-  function(data, valid.effects){
  ...
  data$EFFECT <- gsub("^truncating$","Truncating", ignore.case = TRUE, data$EFFECT)
  ...
  data$EFFECT <- gsub("^splicing$|^splice_site$","Splicing", ignore.case = TRUE, data$EFFECT)
  
  data$EFFECT <- gsub("^initiator_codon_change$|^translation_start_site$","initiator_codon_change", ignore.case = TRUE, data$EFFECT)
  
  #data$EFFECT <- gsub("^inframe_codon_loss$|^inframe_indel$|^inframe_deletion$|^inframe_codon_gain$|^inframe_insersion$|^inframe_variant$|^in_frame_del$|^in_frame_ins$|^inframe$","inframe_indel", ignore.case = TRUE, data$EFFECT)
  data$EFFECT <- gsub("^inframe_codon_loss$|^inframe_indel$|^inframe_deletion$|^inframe_codon_gain$|^inframe_insersion$|^inframe_variant$|^in_frame_del$|^in_frame_ins$|^inframe$","inframe", ignore.case = TRUE, data$EFFECT)
  ...
  # data$EFFECT <- gsub("^frameshift_indel$|^frameshift_del$|^frameshift_variant$|^frame_shift_del$|^frame_shift_ins$|^frameshift$","frameshift_indel",ignore.case = TRUE, data$EFFECT)
  data$EFFECT <- gsub("^frameshift_indel$|^frameshift_del$|^frameshift_variant$|^frame_shift_del$|^frame_shift_ins$|^frameshift$","frameshift",ignore.case = TRUE, data$EFFECT)
