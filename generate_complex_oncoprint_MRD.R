generate_complex_oncoprint_MRD <-  function(muts= muts, cnvs= NULL, svs= NULL ,  # define variants DFs
  ...
  A <- sort_variants(muts, muts.order, gene.list = NULL, group.label= "Substitusions/Indels", variants.class= "Mutations")
  
  data <- A[[1]][,c("TARGET_NAME","GENE","EFFECT","BI")]
  ...
  if (!is.null(cnvs)){
    ...
    D <- sort_variants(cnvs, cnvs.order, gene.list, group.label= "CNVs", variants.class= "CNVs")
  ...
  alter_fun = list(
    ...
    bi_hit= function(x, y, w, h) {
      grid.segments(x - w*0.5, y - h*0.5, x + w*0.5, y + h*0.5, gp = gpar(col="red", lwd = 5))},
  ...
  EFFECT <- list(variants = c("missense","stop_gain","frameshift_indel",
                              ...
                              "add","der",
                              "other_snvs",
                              "multi_hit","biallelic_alteration","inconclusive","normal"), 
  ...
  if (show.multis){  
    ...
    ht.list = list(Legend(labels =  c(">1 variant","Biallelic alteration"),
                          labels_gp = gpar(fontsize = legend.label.font),
                          type = "points",
                          pch = c(21,26),
                          size = unit(1, "cm"),
                          #legend_gp = gpar(col = 4:5, lty = 1:2),
                          #legend_gp = c(gpar(col = "green", fill= "#FAEFD1", lwd= 2, fontsize = legend.label.font)),
                          legend_gp= gpar(col = c("black","red"), fill= "#FAEFD1", lwd= c(2,5)),
                          background = NULL, 
  ...
  if (is.null(save.name)){
    saveFile <- file.path(savePath,paste0("Basic_Heatmap_TEMP_minFreq_",min.freq,".jpg"))
  } else {
    saveFile <- file.path(savePath,paste0("Basic_Heatmap_TEMP_minFreq_",min.freq,"_",save.name,".jpg"))
  }
  ...
  if (show.multis){
    ...
    bi.hit <- data %>% filter(BI==1) %>% select(TARGET_NAME, GENE) %>% unique()
    ...
    if (nrow(multi.hits)>0){
      ...
      for (k in 1: nrow(bi.hit)){
        M[as.character(bi.hit$GENE[k]), as.character(bi.hit$TARGET_NAME[k])] <- paste0(M[as.character(bi.hit$GENE[k]), as.character(bi.hit$TARGET_NAME[k])], "bi_hit",";", collapse = "")
      }
    }
  }
